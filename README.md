...
> 📊 프로젝트 전체를 설명하는 **Figure 17종**은 [`figures/`](figures/)에 정리되어 있습니다 — 모두 이 레포의 실제 산출물에서 계산했습니다.
...
<img src="figures/png/02_problem_setup.png" width="860">

<sub>문제 설정 · taxonomy 구조 — 정답이 root-to-leaf 경로라는 성질이 예측 공간을 2⁵³¹→568로 축소한다 (results/phase0.json)</sub>
...
<img src="figures/png/01_pipeline.png" width="860">

<sub>전체 파이프라인 — 하나의 NLI 점수 행렬에서 선생·답지·학생 세 갈래가 갈라진다</sub>
...
<img src="figures/png/04_core_mining.png" width="860">

<sub>Core class mining — 상대 확신도 conf(D,c) + 이중 필터로 silver label 채굴</sub>
...
<img src="figures/png/05_classifier_arch.png" width="860">

<sub>분류기 구조 — 문서 인코더(BERT [CLS]) · 클래스 인코더(GCN) · bilinear 매칭</sub>
...
<img src="figures/png/06_path_decoding.png" width="860">

<sub>Path decoding — 568 candidate path 집계로 argmax 경로 제출, top-3 대비 +19pt</sub>
...
<img src="figures/png/03_eval_protocol.png" width="860">

<sub>평가 프로토콜 — test GT는 채점 전용, train GT는 감사 전용, 모델 제작은 GT를 못 본다</sub>
...
<img src="figures/png/12_perf_trajectory.png" width="860">

<sub>성능 궤적 — 모델 고정, 데이터만 개선으로 0.3995→0.5488 (논문 NoST 상회)</sub>
...
<img src="figures/png/07_metric_masking.png" width="860">

<sub>지표 반전 — 문서 단위 71%가 라벨 단위 30.3%를 숨겼다 (문서당 3.8개 중 2.7개 오답)</sub>
...
<img src="figures/png/10_precision_coverage.png" width="860">

<sub>Precision–Coverage 곡선 — 무릎점(0.15)을 곡선 위에서 선택</sub>


<img src="figures/png/08_overfitting.png" width="860">

<sub>과적합 시그니처 — silver를 외울수록(train fit↑) test 정체 → 교정 후 함께 상승</sub>
...
<img src="figures/png/14_llm_budget.png" width="860">

<sub>LLM 예산의 임계 규모 — 1K는 무효(0pt), 10K는 유효(+5.7pt)</sub>
...
<img src="figures/png/15_selftraining_stopping.png" width="860">

<sub>Self-training stopping — validation이 test와 같은 지점(batch 200)에서 최고 → 0.5488 확보</sub>
...
<img src="figures/png/17_repro_notes.png" width="860">

<sub>재현 노트 — 논문을 문자 그대로 구현하면 조용히 왜곡되는 5가지 이슈와 대응</sub>
...
<img src="figures/png/13_precision_f1_comovement.png" width="860">

<sub>Precision–F1 동행 — precision이 오른 구간에서만 F1이 올랐다 (병목=데이터의 정량 증거)</sub>
